...
# After the line where you group the data:
cluster = data2.groupby(by=data2['kmeans'], sort=False)  # Add sort=False to avoid shape mismatch issue

# Modify the lambda function to sort values directly without using apply:
df1 = cluster.apply(lambda x: x.sort_values(by="popularity", ascending=False))

# Reset index after sorting:
df1.reset_index(drop=True, inplace=True)  # Reset index without keeping the original index

# Check if df1 is sorted properly
print(df1.head())  # Print the first few rows to verify the sorting
